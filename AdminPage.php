...
function jfb_output_premium_panel_tease()
{
    ...
    <form name="formPremOptions" method="post" action="">
		...
		<input <?php disableatt() ?> type="radio" name="<?php echo $opt_jfbp_restrict_reg; ?>" value="4" <?php echo (get_option($opt_jfbp_restrict_reg)==4?"checked='checked'":"")?>><?php _e('Membership - Only autoregister members of Facebook group id:', 'wp-fb-ac')?> <input <?php disableatt() ?> type="text" size="15" name="<?php echo $opt_jfbp_restrict_reg_gid?>" value="<?php echo get_option($opt_jfbp_restrict_reg_gid); ?>" /> <?php jfb_output_simple_lightbox("", sprintf(__("To find a group's id, view its URL.  It will be something like '%s'.  In this example, the group id would be 12345678.", 'wp-fb-ac'), "http://www.facebook.com/group.php?gid=12345678") . "<br/><br/>" . __("In order to use this option, you'll need to submit your app for review by Facebook.  Please see FAQ#56 for more info:", 'wp-fb-ac') . " <a href='$jfb_homepage#faq56' target='extendedfaq'>here</a>")?><br />
